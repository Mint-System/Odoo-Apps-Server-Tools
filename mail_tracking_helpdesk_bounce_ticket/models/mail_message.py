...
class MailMessage(models.Model):
    ...
    is_bounced_message = fields.Boolean(compute="_compute_is_bounced_message", store=True)
    ...
    def _compute_is_bounced_message(self):
        ...
        for message in self:
            has_bounced_trackings = bounced_states.intersection(message.mapped("mail_tracking_ids.state"))
            message.is_bounced_message = has_bounced_trackings

            if message.is_bounced_message and message.message_type == "comment" and message.subtype_id == subtype_id:
